...
const nickname = document.getElementById("fullname").value

function getCookie(name) {
    const value = `; ${document.cookie}`;
    const parts = value.split(`; ${name}=`);
    if (parts.length === 2) return roomNumber = parts.pop().split(';').shift();
}
...
chatForm.addEventListener("submit", (e) => {
    ...
    if (messageInput.value) {
        socket.emit("chat message", { message: messageInput.value, name: nickname, roomNumber });
        messageInput.value = "";
    }
});
...
socket.on("online", (users) => {
    ...
    for (const socketId in users) {
        if (roomNumber === users[socketId].roomNumber) {
            onlineUsers.innerHTML += `
            <li >
                <button type="button" class="btn btn-light mx-2 p-2" data-toggle="modal" data-target="#pvChat" data-id=${socketId} data-client=${users[socketId].nickname
                }
